...
use WPEventsManager\Databases\WPEMSPostDB;
use WPEventsManager\Databases\WPEMSPostMetaDB;
use WPEventsManager\Filters\WPEMSPostMetaFilter;
use WPEventsManager\Filters\WPEMSPostTypeFilter;
...
class PostModel {
	...
	public static function get_item_model_from_db( $filter ) {
		...
		try {
			...
			$wpems_post_db->get_query_single_row( $filter );

			$query_single_row = $wpems_post_db->get_posts( $filter );
			$post_rs          = $wpems_post_db->wpdb->get_row( $query_single_row );
	...
	public function get_all_metadata() {
		if ( empty( $this->is_got_meta_data ) ) {
			...
			$metadata_rs = $wpems_item_meta_db->get_post_metas( $filter );
	...
	public function get_categories(): array {
		...
		$categories = get_the_terms( $wpPost, 'tp_event_category' );
		if ( ! $categories ) {
			$categories = array();
		}
