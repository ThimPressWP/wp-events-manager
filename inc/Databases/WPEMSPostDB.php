...
use WPEventsManager\Helper\WPEMSHelper;

class WPEMSPostDB extends WPEMSDatabase {
	 ...
	 * @version 1.0.0
	 */
	public function get_posts( WPEMSPostTypeFilter $filter, int &$total_rows = 0 ) {
		...
		if ( ! empty( $filter->post_status ) ) {
			$post_status_format = WPEMSHelper::db_format_array( $filter->post_status, '%s' );
		...
		if ( ! empty( $filter->post_authors ) ) {
			$post_authors_format = WPEMSHelper::db_format_array( $filter->post_authors, '%d' );
