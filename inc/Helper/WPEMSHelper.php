...
namespace WPEventsManager\Helper;
...
class WPEMSHelper {
	...
	public static function getUrlCurrent(): string {
		...
		$http_host   = WPEMSHelper::sanitize_params_submitted( urldecode( $_SERVER['HTTP_HOST'] ?? '' ) );
		$request_uri = WPEMSHelper::sanitize_params_submitted( urldecode( $_SERVER['REQUEST_URI'] ?? '' ) );
		//$http_host   = WPEMSHelper::sanitize_params_submitted( filter_input( INPUT_SERVER, 'HTTP_HOST' ) ?? '' );
		//$request_uri = WPEMSHelper::sanitize_params_submitted( filter_input( INPUT_SERVER, 'REQUEST_URI' ) ?? '' );
