...
//        add_action( 'tp_event_schedule_status', array( __CLASS__, 'schedule_status' ), 10, 2 );
    ...
    public static function save( $post_id, $posted ) {
        ...
        remove_action( 'event_process_update_tp_event_meta', array( __CLASS__, 'save' ), 10, 3 );
        ...
        if ( ( $start && !$end ) || ( strtotime( $start ) >= strtotime( $end ) ) ) {
            Event_Admin_Metaboxes::add_error( __( 'Please make sure end time after start time', 'tp-event' ) );
        ...
        add_action( 'event_process_update_tp_event_meta', array( __CLASS__, 'save' ), 10, 3 );
    }
