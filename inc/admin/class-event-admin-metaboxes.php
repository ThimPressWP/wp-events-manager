...
class TP_Event_Admin_Metaboxes {
	...
	public static function add_meta_boxes() {
		...
		add_meta_box(
			'booking-information', __( 'Booking Information', 'tp-event' ), array( 'TP_Event_Admin_Metabox_Booking', 'render' ), 'event_auth_book', 'normal', 'default'
		);
	}
	...
	public static function save_post_meta( $post_id ) {
		...
		if ( $post_type == 'tp_event' && ( empty( $_POST['event-nonce'] ) || !wp_verify_nonce( $_POST['event-nonce'], 'event_nonce' ) ) ) {
			return false;
		} elseif ( $post_type == 'event_auth_book' && ( empty( $_POST['event-booking-nonce'] ) || !wp_verify_nonce( $_POST['event-booking-nonce'], 'event-booking-nonce' ) ) ) {
			return false;
		}
